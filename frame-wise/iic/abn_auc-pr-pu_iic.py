...
parser.add_argument('--auc_weight', default=1, type=int,
                    metavar='W', help='att_loss1_weight(default: 1)')
parser.add_argument('--iic_weight', default=1, type=int,
...
def main():
    ...
    if args.resume:
        ...
        logger1 = Logger('auc-pr-pu_iic-train'+str(args.val_num)+"_seed"+str(args.manualSeed)+".txt", title=title)
        logger1.set_names(['auc_loss_att','auc_loss_per','mse_loss','iic_loss','top1' ])
        logger2 = Logger('auc-pr-pu_iic-test'+str(args.val_num)+"_seed"+str(args.manualSeed)+".txt", title=title)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # append logger file
        logger1.append([auc_loss_att,auc_loss_per, mse_loss, iic_loss,top1])
...
def train(train_loader, model,criterion_auc, criterion_mse,optimizer, epoch, use_cuda):
    ...
    for batch_idx, (image_t1,image_aug_t1,image_aug_t2, targets) in enumerate(train_loader):
        ...
        iic_loss = IID_loss(iic_prob1, iic_aug_prob1)

        mse_att_loss = criterion_mse(att_aug1,att_aug2.detach())
        mse_per_loss = criterion_mse(per_aug1,per_aug2.detach())

        positive_loss_att, negative_loss_att, lambda_term_att = criterion_auc(att_aug1, targets)
        positive_loss_per, negative_loss_per, lambda_term_per = criterion_auc(per_aug1, targets)
        ...
        loss = args.auc_weight*auc_loss + \
               args.mse_att_weight*mse_att_loss + args.mse_per_weight*mse_per_loss + \
               args.iic_weight*iic_loss
        ...
        auc_losses_per.update(auc_loss_per, image_t1.size(0))
        auc_losses_att.update(auc_loss_att, image_t1.size(0))
        mse_losses.update(mse_per_loss, image_t1.size(0))
        iic_losses.update(iic_loss.data, image_t1.size(0))
        top1.update(prec1, image_t1.size(0))

        # plot progress
        bar.suffix  = '({batch}/{size})|auc_loss: {auc_loss:.4f}|mse_loss: {mse_loss:.4f}|iic_loss: {iic_loss:.4f} |top1: {top1: .4f} | '.format(
                    batch=batch_idx + 1,
        ...
        bar.next()
    bar.finish()
    return (auc_losses_att.avg,auc_losses_per.avg,mse_losses.avg,iic_losses.avg,top1.avg)
...
def val(val_loader, model,criterion_auc, criterion_mse, use_cuda):
    ...
    for batch_idx, (image_t1,image_aug_t1, targets) in enumerate(val_loader):
        ...
        # plot progress
        bar.suffix  = '({batch}/{size})|auc_loss: {auc_loss:.4f}|iic_loss: {iic_loss:.4f} |top1: {top1: .4f} | '.format(
                    batch=batch_idx + 1,
                    size=len(val_loader),
                    auc_loss=auc_losses_per.avg,
                    iic_loss=iic_losses.avg,
...
def save_checkpoint(state,checkpoint='checkpoint', filename='auc-pr-pu_iic'+str(args.val_num)+"_mse_iic"+"seed"+str(args.manualSeed)+"_beta"+str(args.beta)+".pth.tar"):
