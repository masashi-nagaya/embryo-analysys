...
parser.add_argument('--data_dir', default="/home/masashi_nagaya/M2/dataset_9_08/", type=str,
                    help='data directory')  
...
###train###
for i in range(5):
    with open('./csv/train'+str(i+1)+'.csv', 'w') as f:
        writer = csv.writer(f)
...
for i in range(5):
    ###val###
    with open('./csv/test'+str(i+1)+'.csv', 'w') as f:
        writer = csv.writer(f)
...
for i in range(5):
    ###val###
    with open('./csv/test'+str(i+1)+'_last.csv', 'w') as f:
        writer = csv.writer(f)
        for movie in val_birth[i]:
            img_list = []
            for img in os.listdir(dir+movie):
